...
export default function CustomChart({ convertAmount, loading, data, pair, bidMarkers = [], TIMEFRAME_SEC = 60, chartType = "candle", userSetting, onLoadMore }) {
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            // Vertical Grid — candleX ke saath sync hoti hain
            // (stepW se repeat, slideW+panPixelOffset se shift — same as candleX formula)
            // NOTE: slideW aur panPixelOffset candleX ke baad define hote hain, 
            //       isliye vertical grid bhi candleX ke baad draw hogi — GRID_V label dekho
            ...
            // GRID_V — Vertical grid lines: har candle slot pe ek line, candleX ke saath move
            {
                const subCols = isMobile ? 3 : 4;
                const totalShift = slideW + panPixelOffset;
                // pehli line ki x: padding.left se shuru, totalShift % stepW se offset
                const firstX = padding.left + (stepW - (totalShift % stepW)) % stepW;
                // pehli line ka column number (major/minor check ke liye)
                let colIndex = Math.ceil(totalShift / stepW);

                ctx.save();
                ctx.beginPath();
                ctx.rect(padding.left, padding.top, chartW, chartH);
                ctx.clip();
                for (let x = firstX; x <= padding.left + chartW; x += stepW, colIndex++) {
                    const isMajor = colIndex % subCols === 0;
                    ctx.beginPath();
                    ctx.moveTo(Math.floor(x), padding.top);
                    ctx.lineTo(Math.floor(x), padding.top + chartH);
            ...
            // X Axis Time Labels — startIdx+i se anchor, candleX(i) pe draw (candle+grid ke saath move)
            ...
            visible.forEach((c, i) => {
                // global candle index ke basis pe decide karo label dikhana hai ya nahi
                if ((startIdx + i) % labelEvery !== 0) return;
                const x = candleX(i);
            ...
            if (liveCandle && panRef.current.offset === 0) {
                ...
                const serverOffset = liveCandle.time; // server ne last candle kab start ki
                const elapsed = nowSec - serverOffset; // PC clock se elapsed (approximate)
                const remaining = Math.max(0, currentTF - (elapsed % currentTF));
    ...
    useEffect(() => {
        ...
        const handleMouseMove = (e) => {
            ...
            if (!mouseRef.current.down) return;
            const totalDx = x - mouseRef.current.startX;
            // 🔥 increased from 2 → 3 (300px feel)
            const sensitivity = 3;
            const sw = stepWRef.current > 0 ? stepWRef.current : 20;

            const newOffset =
                mouseRef.current.startOffset + (totalDx * sensitivity / sw);

            const candles = dataRef.current;
            const vc = Math.floor(zoomRef.current.visibleCount);

            panRef.current.offset = Math.max(
                0,
                Math.min(newOffset, Math.max(0, candles.length - vc))
            );
            // 🔥 slightly smoother velocity for longer drag feel
            velocityRef.current = e.movementX * 0.4;
            panRef.current.dragging = true;
            panRef.current.userInteracted = true;
        ...
        const startInertia = () => {
            ...
            const animate = () => {
                ...
                const candles = dataRef.current;
                const vc = Math.floor(zoomRef.current.visibleCount);
                // 🔥 slightly stronger glide
                panRef.current.offset += v / 4;

                panRef.current.offset = Math.max(
                    0,
                    Math.min(panRef.current.offset, Math.max(0, candles.length - vc))
                );

                // friction (slightly smoother)
    ...
    const handleGoLive = () => {
        const candles = dataRef.current;
        if (!candles.length) return;
        const startOffset = panRef.current.offset;
        const duration = 1000;
        ...
        const animate = (time) => {
            ...
            panRef.current.offset =
                startOffset + (0 - startOffset) * ease;
